...
module.exports = function(app) {
	...
	var photo_album_tree,
			_filename = __dirname + "/upload_pics.json",
			folderPath = path.join(__dirname, '../..', 'public/media/');
	...
	app.put("/v1/albums.json", function (req, res) {
		console.log("inside put albums : ", req.body);
	...
	app.put("/v1/albums/:album_name/photos.json", function (req, res) {
		console.log("inside put album photos.json");
		if (!req.body || !req.body.filename){
			console.log("body & filename ", req.body, req.filename);
	...
	function mkdirs_for_album(album_name, callback) {
		...
		var dirs = [
					...
					folderPath + album_name + "/thumb",
					folderPath + album_name + "/full"
				];
	...
	function copy_file_to_destinations(uploadfn, album_name, destfn, callback) {
		console.log("copy_file_to_destinations");
		var np1 = folderPath + album_name + "/thumb/" + destfn;
		var np2 = folderPath + album_name + "/full/" + destfn;
		...
		copy_file(uploadfn, np1, function (err, results) {
			...
			console.log("copy_file_to_destinations - copyfile 1");
			copy_file(uploadfn, np2, function (err) {
				...
				console.log("copy_file_to_destinations - copyfile 2");
